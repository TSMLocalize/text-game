    ...
    void Start()        
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                if (y % 2 == 0)
                {
                    tileGO = Instantiate(tile, new Vector3((this.transform.position.x + x) * XoffSet, (this.transform.position.y + y) * YoffSet), Quaternion.identity, this.gameObject.transform);
                }
                else
                {
                    tileGO = Instantiate(tile, new Vector3((this.transform.position.x + x + 0.5f) * XoffSet, (this.transform.position.y + y) * YoffSet), Quaternion.identity, this.gameObject.transform);
                }
